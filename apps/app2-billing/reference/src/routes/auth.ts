...
export function authRoutes(config: AppConfig): Router {
  ...
  /** V2.5 fix — randomBytes token, TTL, one-time; token not returned */
  r.post('/api/reset/request', async (req, res) => {
    ...
    if (user.rows[0]) {
      ...
      await query(
        `UPDATE users SET reset_token = $2, reset_expires = (NOW() AT TIME ZONE 'utc') + INTERVAL '15 minutes'
         WHERE id = $1`,
