...
- **StorageClass:** local-path (встроен в k3s) для PVC per-team `workspace` / `postgres-data` /
  `code-server-data` и platform `grafana-data`.
- **TLS:** `bootstrap-cluster.sh` ставит self-signed default-cert в ingress-nginx (`hack-local-tls`)
  с SAN `hack.local`, `*.hack.local`, `*.app.hack.local`, `*.ide.hack.local` (wildcard X.509 —
  один уровень; `*.hack.local` сам по себе не покрывает `team-a.app.hack.local`). Ingress'ы стенда/
  Grafana/scoreboard включают `ssl-redirect` + `force-ssl-redirect`.
...
- `deploy/k8s/team-template` — параметризованный шаблон стенда: `Deployment(app)` + `StatefulSet(postgres)`
  + `Deployment(code-server)` + `Service` + `PVC(workspace)` + `PVC(code-server-data)` +
...
- code-server запускается под этим же ограниченным ServiceAccount — терминал команды не может трогать чужое.
  Образ `hacktraining/code-server:local` (`platform/code-server/Dockerfile`) вшивает ESLint / Prettier /
  GitLens через `--install-extension <local.vsix>` в `/opt/hacktraining/extensions` (отдельно от PVC
  `code-server-data`, чтобы настройки переживали рестарт, а расширения не затирались пустым volume).

### 8.5 Локальная разработка
- `deploy/docker/compose.dev.yml` поднимает app1/app2 (vuln+ref) + postgres + scoreboard/checker/planter/
  internal-metadata без k8s — для разработки и **проверки эталонных эксплойтов** (`tools/attacker-scripts`).
  Конфиг скоринга в сети compose: `platform/scoreboard/config.compose.yaml`.
  Запуск: `docker compose -f deploy/docker/compose.dev.yml up --build -d` (см. `deploy/docker/README.md`).
...
| IDE команд | `hacktraining/code-server:local` (база `codercom/code-server:4.109.5` +
  офлайн ESLint/Prettier/GitLens в `/opt/hacktraining/extensions`) | Правка в браузере, RBAC на namespace |
...
Актуальный статус с доказательствами — [`docs/DOD.md`](DOD.md). Критерии:

- [x] Оба приложения запускаются локально (`compose.dev.yml`) и описаны для k3s (`deploy/k8s`).
- [x] app1: `vulnerable/` и `reference/` используют идентичный `src/`; отличия только в конфиге.
- [x] app2: `reference/` закрывает все заявленные уязвимости; `vulnerable/` — воспроизводимо уязвим.
- [x] Для каждой уязвимости есть рабочий PoC в `tools/attacker-scripts` и запись в `SOLUTION.md`
      (эксплуатация + фикс + пример + след в логах).
- [x] flag-planter раскладывает флаги; scoreboard принимает и начисляет; checker меряет SLA — end-to-end.
- [x] Логи всех подов собираются в Loki; 5 дашбордов Grafana провижнятся автоматически (манифесты platform).
- [x] `reset-round.sh` полностью восстанавливает уязвимый baseline < 60 c.
- [x] NetworkPolicy подтверждённо блокирует доступ атакующих к postgres/code-server/платформе.
- [x] `docs/instructor-guide.md` описывает роли, тайминг, подсказки и чек-лист судьи.

Приёмочный smoke: `scripts/phase8-smoke.sh` / `phase8-smoke.ps1` → `artifacts/phase8-smoke-report.md`.
